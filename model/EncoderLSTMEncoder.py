...
class EncoderLSTMEncoder(LSTM):
    def __init__(self, input_size=5, hidden_layer_size=100, *args, **kwargs):
        ...
        self.encoder = nn.LSTM(hidden_layer_size, hidden_layer_size)
        self.linear2 = nn.Linear(hidden_layer_size, input_size * 1)
    ...
    def forward(self, batch):
        ...
        input = []
        for i in range(0, 5):
            input.append(self.linear1(batch[i]))
        encoder_outputs, hidden_cell = self.encoder(cat(input).view(5, 1, 100),
                                                    (self.hidden_cell_1, self.hidden_cell_2))
        output = self.linear2(encoder_outputs[-1])
        return output
